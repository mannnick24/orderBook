...
class OrderBook {
  ...
  /** a reference to the master map by id **/
  ...
  void add( Order order ) {
    ...
    /*
     * we must have a lock on both the id map and the level map to keep them in sync
     * MUST ensure no code calls sync in the opposite way round
     */
    synchronized( ordersById ) {
      synchronized( ordersByLevel ) {
  ...
  boolean modify( Order order, long newQuantity )
  {
  ...
  /*
   * in this version, we need to move to the end of the queue
