...
class OrderValidater {
  ...
  void modifyOrder(String orderId, long qty) {
    ...
    Order modified = new Order.Builder().clone(validatingOrders.get(orderId)).setQuantity(qty).get();
    // keep it for later
  ...
  void validate(long bestPrice, long level, long orderNumber, long qty, long vol) {
    assertEquals(String.format("Best price is %s", bestPrice), Optional.of(bestPrice),
        obManager.getBestPrice(instrument, side));
    assertEquals(String.format("orders at %s is %s", level, orderNumber), orderNumber,
        obManager.getOrderNumAtLevel(instrument, side, level));
    assertEquals(String.format("Total qty at %s is %s", level, qty), qty,
        obManager.getTotalQuantityAtLevel(instrument, side, level));
    assertEquals(String.format("Total vol at %s is %s", level, vol), vol,
